...
void transpuesta(Matrix M, Matrix &T){
	zeroes(T, M.size());
	for(int i=0; i<M.size(); i++)
		for(int j=0; j<M.at(0).size(); j++)
...
void cofactores(Matrix M, Matrix &cof){
	zeroes(cof, M.size());
	for(int i=0; i<M.size(); i++){
		for (int j = 0; j<M.at(0).size(); j++)
		{
...
void adjunta(Matrix M, Matrix &adj){
	Matrix Cofactores;
	cofactores(M, Cofactores);
	transpuesta(Cofactores, adj);
} 
